...
class word_parser {
    ...
    public function __construct($sector = '', $valid_competencies = []) {
        ...
        // Es: AUTOMOBILE_MAu_A3 → AUTOMOBILE_MAU_A3
    ...
    private function parse_questions() {
        ...
        // Pattern per trovare domande con vari formati:
        // - Q01, Q02, Q1 (semplice)
        // - **Q01** (grassetto markdown)
        // - Q01 (CODICE) (con codice in parentesi)
        // - AUT_BASE_Q01, CHIM_OBB_Q01, MECCANICA_Q01 (con prefisso settore)
        // - Domanda 1, Domanda 01 (formato italiano)
        $pattern = '/(?:^|\n)\s*(?:\*\*)?(?:[A-Z_]+_)?Q(\d+)(?:\s*\([^)]+\))?(?:\*\*)?\s*(?:\n|$)/i';
        
        // Split per domande
        $parts = preg_split($pattern, $text, -1, PREG_SPLIT_DELIM_CAPTURE);
        
        // Se non trova domande con Q01, prova con "Domanda 1"
        if (count($parts) <= 1) {
            $pattern = '/(?:^|\n)\s*(?:\*\*)?Domanda\s+(\d+)(?:\*\*)?\s*(?:\n|$)/i';
            $parts = preg_split($pattern, $text, -1, PREG_SPLIT_DELIM_CAPTURE);
        }
    ...
    private function extract_competency($content) {
        ...
        // FORMATO 3: Competenza collegata: AUTOMOBILE_MR_A1 (formato Autoveicolo)
        if (preg_match('/Competenza\s+collegata:\s*(\S+)/i', $content, $m)) {
            $result['code'] = $this->clean_competency_code($m[1]);
            return $result;
        }
        
        // FORMATO 4: Competenza: SETTORE_XXX (senza pipe, a fine riga)
        if (preg_match('/Competenza:\s*([A-Z]+_[A-Z0-9_]+)/i', $content, $m)) {
            $result['code'] = $this->clean_competency_code($m[1]);
            return $result;
        }
        
        // FORMATO 5: Cerca pattern SETTORE_XXX_YY ovunque nel testo
        ...
        // FORMATO 6: Pattern generico SETTORE_XX_YY (più permissivo)
    ...
    private function extract_answers($content) {
        ...
        foreach ($lines as $line) {
            ...
            // Pattern più ampio: A) A. A- A: o solo "A " seguito da testo
        ...
        if (empty($answers)) {
            // Cerca pattern tipo "A) testo" o "A. testo" anche in mezzo al testo
        ...
        if (empty($answers)) {
            ...
            foreach ($lines as $line) {
                $line = trim($line);
                // Cerca linee che iniziano con bullet
