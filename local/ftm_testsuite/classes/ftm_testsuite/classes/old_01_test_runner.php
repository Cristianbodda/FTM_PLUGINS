 ...
 * 
 * VERSIONE 2.2 - Aggiornata 09/01/2026
 * 
 * CORREZIONE v2.2:
...
class test_runner {
    ...
    public function run_all($runname = '') {
        $fw_name = $this->get_framework_name();
        $this->manager->start_run($runname ?: "Test Completo - Framework: {$fw_name} - " . date('d/m/Y H:i'));
    ...
    private function test_1_1_questions_with_competencies() {
        ...
        // Costruisco la query con filtro framework opzionale
        $framework_join = "";
        $framework_where = "";
        $params = [];
        
        if ($this->frameworkid > 0) {
            $framework_join = "JOIN {competency} c_filter ON c_filter.id = qc.competencyid";
            $framework_where = "AND c_filter.competencyframeworkid = ?";
        ...
        $fw_info = $this->frameworkid > 0 ? " (Framework: {$this->get_framework_name()})" : "";
        $this->manager->record_result(
            'quiz', '1.1', 'Domande con competenze',
            $status, '100%', $percentage . '%',
            "{$result->questions_with_comp} domande su {$result->total_questions} hanno competenze assegnate.{$fw_info}",
    ...
    private function test_1_2_competencies_exist() {
        ...
        $sql = "SELECT COUNT(*) as orphans 
                ...
                WHERE c.id IS NULL
                " . ($this->frameworkid > 0 ? "OR c.competencyframeworkid != ?" : "");
        
        if ($this->frameworkid > 0) {
            $params[] = $this->frameworkid;
        }

        // Query semplificata: conta solo orfani reali
        $sql = "SELECT COUNT(*) as orphans 
                FROM {qbank_competenciesbyquestion} qc 
                LEFT JOIN {competency} c ON c.id = qc.competencyid 
                WHERE c.id IS NULL";
    ...
    private function test_1_4_orphan_questions() {
        ...
        // Se c'è un framework selezionato, le domande "orfane" sono quelle senza competenze
        // di QUEL framework (potrebbero avere competenze di altri framework)
        $params = [];
        ...
        $status = $result->orphans == 0 ? 'passed' : 'failed';
        $fw_info = $this->frameworkid > 0 ? " (senza competenze del framework {$this->get_framework_name()})" : "";
        $this->manager->record_result('quiz', '1.4', 'Domande orfane', $status, '0', $result->orphans,
            $result->orphans == 0 ? 'Tutte le domande hanno competenze.' : "{$result->orphans} domande senza competenze!{$fw_info}",
            $sql, [], microtime(true) - $start);
    }
    ...
    private function test_1_5_responses_recorded() {
        ...
        $sql = "SELECT COUNT(*) as attempts FROM {quiz_attempts} qa WHERE qa.userid = ? AND qa.state = 'finished'";
    ...
    private function test_1_7_manual_calculation() {
        ...
        // Prendi una competenza a caso testata dall'utente (del framework selezionato)
    ...
    private function run_module_labeval() {
        ...
        // Test IMPLEMENTATI in v2.1
        $this->test_3_6_score_calculation();
        $this->test_3_7_cache_updated();
        
        // Test NUOVI in v2.1 per LabEval v2.0
    ...
    /**
     * Test 3.6: Calcolo punteggio pesato
     * IMPLEMENTATO v2.1 - 09/01/2026
     */
    private function test_3_6_score_calculation() {
        ...
        // Prendi le ultime 10 sessioni complete per il test
        ...
        foreach ($sessions as $session) {
            // Verifica se esistono rating con competencycode (v2.0)
            ...
            if ($has_competencycode > 0) {
                // Calcolo v2.0: direttamente dai rating
                ...
                // Calcolo legacy: join con behavior_comp per ottenere i pesi
                $calc = $DB->get_record_sql("
    ...
    /**
     * Test 3.7: Cache comp_scores aggiornata
     * IMPLEMENTATO v2.1 - 09/01/2026
     */
    private function test_3_7_cache_updated() {
        ...
        // Conta sessioni con almeno un record in comp_scores
        ...
        // Verifica record comp_scores con percentage NULL o fuori range
    ...
    /**
     * Test 3.8: Campo competencycode esiste nella tabella ratings
     * NUOVO v2.1 - 09/01/2026
     */
    private function test_3_8_competencycode_field() {
        ...
        // Verifica se la colonna competencycode esiste
    ...
    /**
     * Test 3.9: Indice unique corretto
     * NUOVO v2.1 - 09/01/2026
     */
    private function test_3_9_unique_index() {
        ...
        // Verifica se ci sono duplicati (sessionid, behaviorid, competencycode)
        ...
        // Verifica anche che possano esistere più rating per lo stesso behavior 
    ...
    /**
     * Test 3.10: Rating con competencycode popolato
     * NUOVO v2.1 - 09/01/2026
     */
    private function test_3_10_ratings_with_competencycode() {
        ...
        // Conta rating con competencycode popolato
        $ratings_with_code = $DB->count_records_sql("
            SELECT COUNT(*) FROM {local_labeval_ratings}
            WHERE competencycode IS NOT NULL AND competencycode != ''
        ");

        // Conta rating senza competencycode (legacy)
        $ratings_legacy = $total_ratings - $ratings_with_code;

        $percentage = round(($ratings_with_code / $total_ratings) * 100, 1);

        // Se ci sono rating recenti (ultimi 7 giorni), dovrebbero avere competencycode
        ...
        // Warning se ci sono rating recenti senza competencycode
    ...
    private function test_4_5_gap_analysis() {
        ...
        // Calcola gap tra autovalutazione e performance quiz
    ...
    private function test_5_2_data_complete() {
        ...
        // Verifica che ci siano dati per tutte e 3 le fonti
    ...
    // ========================================================================
    // MODULO 6: COPERTURA COMPETENZE (NUOVO 04/01/2026)
    // ========================================================================
    ...
    // ========================================================================
    // MODULO 7: INTEGRITÀ DATI (NUOVO 04/01/2026)
    // ========================================================================
    ...
    private function test_7_2_orphan_records() {
        ...
        // Selfassessment con competenze inesistenti (solo del framework selezionato)
        if ($this->frameworkid > 0) {
            $count = $DB->count_records_sql("
                SELECT COUNT(*) FROM {local_selfassessment} sa 
                JOIN {competency} c ON c.id = sa.competencyid
    ...
    // ========================================================================
    // MODULO 8: ASSEGNAZIONI AUTOVALUTAZIONE (NUOVO 04/01/2026)
    // ========================================================================
