...
if ($step == 3):
    ...
    $excel_verification_result = null;
    
    // === GESTIONE UPLOAD EXCEL PER CONFRONTO ===
    if (isset($_FILES['excelfile']) && $_FILES['excelfile']['error'] === UPLOAD_ERR_OK && 
        isset($SESSION->word_import_questions)) {
        
        $excel_tmp = $_FILES['excelfile']['tmp_name'];
        $excel_name = $_FILES['excelfile']['name'];
        
        $verifier = new \local_competencyxmlimport\excel_verifier();
        $load_result = $verifier->load_excel($excel_tmp);
        
        if ($load_result['success']) {
            // Auto-seleziona il primo foglio con dati
            $sheets = $load_result['sheet_names'];
            $selected_sheet = null;
            
            // Cerca foglio che contenga il settore o "QC_"
            foreach ($sheets as $sheet) {
                if (stripos($sheet, $sector) !== false || stripos($sheet, 'QC_') !== false) {
                    $selected_sheet = $sheet;
                    break;
                }
            }
            if (!$selected_sheet && !empty($sheets)) {
                $selected_sheet = $sheets[0];
            }
            
            if ($selected_sheet) {
                $sheet_info = $verifier->select_sheet($selected_sheet);
                
                // Auto-detect colonne: Q, Codice_competenza, Risposta_corretta
                $q_col = $sheet_info['auto_detect']['question_col'] ?? 0;
                $comp_col = $sheet_info['auto_detect']['competency_col'] ?? 1;
                $ans_col = $sheet_info['auto_detect']['answer_col'] ?? 2;
                
                $verifier->set_column_mapping($q_col, $comp_col, $ans_col);
                
                // Esegui verifica
                $excel_verification_result = $verifier->verify($SESSION->word_import_questions);
                $excel_verification_result['excel_filename'] = $excel_name;
                $excel_verification_result['sheet_name'] = $selected_sheet;
                
                // Salva in sessione per usi successivi
                $SESSION->excel_verification_result = $excel_verification_result;
            }
        } else {
            $upload_error .= "❌ Errore Excel: " . $load_result['error'] . "<br>";
        }
    }
    
    // Recupera risultato verifica Excel dalla sessione se presente
    if (!$excel_verification_result && isset($SESSION->excel_verification_result)) {
        $excel_verification_result = $SESSION->excel_verification_result;
    }
            ...
            if ($_FILES['xmlfiles']['error'][$key] === UPLOAD_ERR_OK) {
                ...
                if ($ext === 'docx') {
                    ...
                    if ($word_result['success']) {
                        ...
                        // Pulisci verifica Excel precedente quando si carica nuovo Word
                        unset($SESSION->excel_verification_result);
                        $excel_verification_result = null;
                    } else {
...
if ($step == 5 && $action === 'execute'):
    ...
    $comp_lookup = [];
    foreach ($competencies as $c) {
        // Fix case-sensitivity: normalizza a uppercase per confronto
...
<?php
    ...
    foreach ($quizzes_config as $config) {
        ...
        foreach ($matches[0] as $qxml) {
            ...
            // Estrai testo
            // Prova prima CDATA
            preg_match('/<questiontext[^>]*>\s*<text><!\[CDATA\[(.*?)\]\]><\/text>/s', $qxml, $text_match);
            if (!isset($text_match[1])) {
                // Fallback a HTML entities
            ...
            if ($existing) {
                ...
                $has_answers = $DB->count_records('question_answers', ['question' => $existing->id]);
                if ($has_answers > 0) {
                    // Ha già risposte, usa quella esistente
                    $question_ids[] = $existing->id;
                    continue;
                } else {
                    // NON ha risposte - elimina e ricrea
                    echo '<div class="log-line warning">⚠️ Domanda ' . htmlspecialchars($full_name) . ' senza risposte - ricreazione...</div>';
                    // Elimina la domanda corrotta
                    ...
                    $qv_record = $DB->get_record('question_versions', ['questionid' => $existing->id]);
                    if ($qv_record) {
                        $DB->delete_records('question_versions', ['questionid' => $existing->id]);
                        $DB->delete_records('question_bank_entries', ['id' => $qv_record->questionbankentryid]);
                    }
            ...
            // Risposte - supporta sia CDATA che HTML entities
            // Prima prova CDATA (formato template standard)
            preg_match_all('/<answer fraction="(\d+)"[^>]*>\s*<text><!\[CDATA\[(.*?)\]\]><\/text>/s', $qxml, $answers);
            $is_cdata = true;
            
            // Se non trova CDATA, prova HTML entities
            if (count($answers[0]) == 0) {
                preg_match_all('/<answer fraction="(\d+)"[^>]*>\s*<text>(.*?)<\/text>/s', $qxml, $answers);
                $is_cdata = false;
            }
            
            // DEBUG: se ancora non trova risposte
            if (count($answers[0]) == 0) {
                echo '<div class="log-line error">⚠️ Nessuna risposta trovata per: ' . htmlspecialchars(substr($full_name, 0, 50)) . '</div>';
            }
            
            for ($i = 0; $i < count($answers[0]); $i++) {
                $ans = new stdClass();
                $ans->question = $question->id;
                // Se CDATA, il contenuto è già pronto; altrimenti decodifica HTML entities
                $ans->answer = $is_cdata ? $answers[2][$i] : html_entity_decode($answers[2][$i]);
            ...
            // Assegna competenza (con fix case-sensitivity)
            $comp_code_upper = $comp_code ? strtoupper($comp_code) : null;
            if ($assign_competencies && $comp_code_upper && isset($comp_lookup[$comp_code_upper])) {
                $rec = new stdClass();
                $rec->questionid = $question->id;
                $rec->competencyid = $comp_lookup[$comp_code_upper];
