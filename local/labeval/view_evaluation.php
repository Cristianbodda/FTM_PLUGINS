    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <div class="filter-bar">
                <span class="filter-label">📂 Filtra per area:</span>
                <div class="filter-buttons">
                    <button class="filter-btn active" data-filter="ALL" onclick="updateRadar('ALL')">
                        Tutte (<?php echo count($competencyData); ?>)
                    </button>
                    <?php
                    // Conta competenze per area
                    $areaCount = [];
                    foreach ($competencyData as $code => $data) {
                        $parts = explode('_', $code);
                        $area = isset($parts[1]) ? $parts[1] : 'ALTRO';
                        if (!isset($areaCount[$area])) $areaCount[$area] = 0;
                        $areaCount[$area]++;
                    }
                    ...
                    foreach ($areaCount as $area => $count):
                        $areaLabel = $areaNames[$area] ?? $area;
                    ?>
                    <button class="filter-btn" data-filter="<?php echo $area; ?>" onclick="updateRadar('<?php echo $area; ?>')">
                        <?php echo $areaLabel; ?> (<?php echo $count; ?>)
                    </button>
                    <?php endforeach; ?>
                </div>
            </div>
            ...
            <p style="text-align: center; color: #666; font-size: 12px; margin-top: 10px;">
                <em>💡 Passa il mouse sui punti per vedere i dettagli. Usa i filtri per visualizzare le competenze per area.</em>
            </p>
        </div>
    </div>
...
<?php if (!empty($competencyData)): ?>
// Dati per il radar - prepara etichette descrittive e dati
const allCompetencyData = <?php 
    $radarData = [];
    foreach ($competencyData as $code => $data) {
        // Estrai l'area dal codice (es. MIS da MECCANICA_MIS_01)
        $parts = explode('_', $code);
        $area = isset($parts[1]) ? $parts[1] : 'ALTRO';
...
// Funzione per filtrare e aggiornare il radar
function updateRadar(filter) {
    currentFilter = filter;
    
    // Filtra i dati
    let filteredData = allCompetencyData;
    if (filter !== 'ALL') {
        filteredData = allCompetencyData.filter(d => d.area === filter);
    }
    
    // Prepara labels (descrizioni troncate per leggibilità)
    const labels = filteredData.map(d => {
        // Tronca a 30 caratteri se troppo lungo
    ...
    const values = filteredData.map(d => d.percentage);
                    ...
                    tooltip: {
                        callbacks: {
                            title: function(context) {
                                // Mostra la descrizione completa nel tooltip
                                const idx = context[0].dataIndex;
                                const filtered = currentFilter === 'ALL' ? allCompetencyData : allCompetencyData.filter(d => d.area === currentFilter);
                                return filtered[idx].description;
                            },
                            label: function(context) {
                                const idx = context.dataIndex;
                                const filtered = currentFilter === 'ALL' ? allCompetencyData : allCompetencyData.filter(d => d.area === currentFilter);
                                return [
                                    'Codice: ' + filtered[idx].code,
                                    'Punteggio: ' + context.parsed.r + '%'
                                ];
                            }
                        }
                    }
...
function updateLegend(data) {
    ...
    data.forEach((d, idx) => {
        const colorClass = d.percentage >= 70 ? 'high' : (d.percentage >= 50 ? 'medium' : 'low');
        html += `
            <div class="legend-item">
                <span class="legend-number">${idx + 1}</span>
                <span class="legend-text">${d.description}</span>
                <span class="legend-value ${colorClass}">${d.percentage}%</span>
            </div>
        `;
    });
    html += '</div>';
...
// Inizializza il radar al caricamento
