 ...
 * VERSIONE 2.0: Supporta il nuovo campo competencycode nella tabella ratings
 *
...
function local_labeval_calculate_competency_scores($sessionid) {
    ...
    // Prima controlla se ci sono ratings con competencycode (nuovo sistema)
    $hasNewRatings = $DB->record_exists_sql(
        "SELECT 1 FROM {local_labeval_ratings} WHERE sessionid = ? AND competencycode IS NOT NULL AND competencycode != ''",
        [$sessionid]
    );
    
    if ($hasNewRatings) {
        // NUOVO SISTEMA: usa direttamente il campo competencycode dalla tabella ratings
        // Usa r.id come prima colonna per garantire unicità
        $sql = "SELECT r.id as rid, r.behaviorid, r.rating, r.competencycode,
                       bc.competencyid, bc.weight
                FROM {local_labeval_ratings} r
                LEFT JOIN {local_labeval_behavior_comp} bc 
                    ON bc.behaviorid = r.behaviorid AND bc.competencycode = r.competencycode
                WHERE r.sessionid = ?
        ...
        // VECCHIO SISTEMA: join con behavior_comp per ottenere le competenze
        // Usa CONCAT per creare una chiave unica (behaviorid + competencycode)
        $sql = "SELECT CONCAT(r.behaviorid, '_', bc.competencycode) as uniquekey,
                       r.behaviorid, r.rating, 
    ...
    foreach ($ratings as $rating) {
        ...
        // Usa il peso dal join, default a 1 se non trovato
        $weight = isset($rating->weight) ? (int)$rating->weight : 1;
        ...
        // Score = rating * weight
        $competencies[$code]['score'] += $rating->rating * $weight;
        // Max score = 3 * weight (max rating is 3)
        $competencies[$code]['maxscore'] += 3 * $weight;
...
function local_labeval_update_session_totals($sessionid) {
    ...
    // Get all competency scores for this session
    $scores = $DB->get_records('local_labeval_comp_scores', ['sessionid' => $sessionid]);
...
/**
 * Get common styles for all labeval pages
 *
