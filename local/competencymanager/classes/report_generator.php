<?php
/**
 * Report Generator - Competency Manager
 * Versione unificata con supporto multi-settore
...
class report_generator {
    ...
    public static function student_can_view_own_report($userid, $courseid) {
        ...
        if (!$dbman->table_exists('local_competencymanager_auth')) {
            // Table doesn't exist yet, allow all by default
            return true;
        }
        
        try {
            $auth = $DB->get_record('local_competencymanager_auth', [
                'userid' => $userid,
                'courseid' => $courseid,
                'authorized' => 1
            ]);
    ...
    public static function set_student_authorization($userid, $courseid, $authorized, $authorizedby) {
        ...
        $dbman = $DB->get_manager();
        if (!$dbman->table_exists('local_competencymanager_auth')) {
            // Table doesn't exist, cannot save
            return false;
        }
        
        try {
            $existing = $DB->get_record('local_competencymanager_auth', [
                'userid' => $userid,
                'courseid' => $courseid
            ]);
            ...
            if ($existing) {
                ...
                $DB->update_record('local_competencymanager_auth', $existing);
            } else {
                $record = new \stdClass();
                ...
                $DB->insert_record('local_competencymanager_auth', $record);
            }
