...
class assessment_manager {
     ...
     * Compatibile con Moodle 4.x (nuova struttura question_references)
     */
    public static function generate_colloquio_report($studentid, $quizids = null, $courseid = null) {
        ...
        // 3. Recupera competenze - Query compatibile Moodle 4.x
        $competencies = [];
        if ($courseid) {
            // Query per Moodle 4.x con question_references e question_versions
            ...
            try {
                ...
                // Fallback: prendi tutte le competenze con domande
                $sql = "SELECT DISTINCT c.id, c.idnumber, c.shortname, c.description
                        FROM {competency} c
                        JOIN {qbank_competenciesbyquestion} qbc ON qbc.competencyid = c.id
                        ORDER BY c.idnumber";
        ...
        foreach ($competencies as $comp) {
            ...
            $gap->name = !empty($comp->shortname) ? $comp->shortname : $comp->description;
            ...
            // Cerca nei risultati quiz per competency id O per idnumber
            $found = false;
            $qr = null;
            if (isset($quizResults[$comp->id])) {
                ...
                // Cerca per chiave numerica o stringa
            ...
            if ($found && is_array($qr)) {
                ...
                $gap->quiz_score = $gap->quiz_percentage;
