...
function get_coverage_data_for_export($sector, $courseid) {
    ...
    $sql = "SELECT DISTINCT c.id, c.idnumber, c.shortname,
            SUBSTRING_INDEX(SUBSTRING_INDEX(c.idnumber, '_', 2), '_', -1) as profilo,
            SUBSTRING(SUBSTRING_INDEX(c.idnumber, '_', -1), 1, 1) as area
            FROM {competency} c WHERE c.idnumber LIKE :sector AND c.idnumber NOT LIKE 'old%' ORDER BY c.idnumber";
    $competencies = $DB->get_records_sql($sql, ['sector' => $sector . '_%']);
    $sql_covered = "SELECT DISTINCT c.idnumber FROM {competency} c
                    JOIN {qbank_competenciesbyquestion} qc ON qc.competencyid = c.id WHERE c.idnumber LIKE :sector";
    $covered = $DB->get_records_sql($sql_covered, ['sector' => $sector . '_%']);
    $covered_ids = array_column($covered, 'idnumber');
    if ($courseid) {
        $sql_quiz = "SELECT q.name as quiz_name, c.idnumber as competency,
                    SUBSTRING(SUBSTRING_INDEX(c.idnumber, '_', -1), 1, 1) as area, COUNT(DISTINCT qc.questionid) as count
                    ...
                    WHERE q.course = :courseid AND c.idnumber LIKE :sector GROUP BY q.name, c.idnumber ORDER BY q.name";
        $quiz_data = $DB->get_records_sql($sql_quiz, ['courseid' => $courseid, 'sector' => $sector . '_%']);
        foreach ($quiz_data as $item) {
            $result['questions_by_quiz'][] = ['quiz_name' => $item->quiz_name, 'competency' => $item->competency, 'area' => $item->area, 'count' => $item->count];
        }
    }
    $areas_data = [];
...
function get_missing_competencies($sector) {
    global $DB;
    return $DB->get_records_sql("SELECT DISTINCT c.idnumber, c.shortname, SUBSTRING_INDEX(SUBSTRING_INDEX(c.idnumber, '_', 2), '_', -1) as profilo
        FROM {competency} c LEFT JOIN {qbank_competenciesbyquestion} qc ON qc.competencyid = c.id
        WHERE c.idnumber LIKE :s AND c.idnumber NOT LIKE 'old%' AND qc.id IS NULL ORDER BY c.idnumber", ['s' => $sector.'_%']);
}
