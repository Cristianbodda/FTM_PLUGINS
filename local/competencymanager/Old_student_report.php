...
function get_student_self_assessment($userid, $courseid) {
    ...
    // Verifica se la tabella CoachManager esiste
 ...
 */
function generate_colloquio_hints($gapData, $competencyDescriptions) {
...
if ($showDualRadar || $showGapAnalysis || $showSpuntiColloquio || 
    ...
    if ($autovalutazioneResult) {
        ...
        // Calcola gap se richiesto
        if ($showGapAnalysis || $printGapAnalysis || $showSpuntiColloquio || $printSpuntiColloquio) {
            $gapAnalysisData = calculate_gap_analysis($autovalutazioneData, $competencies);
        }
        
        // Genera spunti colloquio se richiesto
        if ($showSpuntiColloquio || $printSpuntiColloquio) {
            $colloquioHints = generate_colloquio_hints($gapAnalysisData, $competencyDescriptions);
        }
...
// ========================================
    ...
    foreach ($competencies as $c) {
        ...
        if (!isset($uniqueAreas[$aCode])) {
            $uniqueAreas[$aCode] = $areaDescriptions[$aCode] ?? ['name' => $aCode, 'icon' => '📁'];
        }
    }
    ksort($uniqueAreas);
