 ...
 */
function parse_moodle_xml($xmlcontent) {
    ...
    foreach ($xml->question as $q) {
        ...
        $code = null;
        if (preg_match('/([A-Z]+_[A-Z]+_\d+)/i', $name, $m)) {
            $code = strtoupper($m[1]);
        }
    ...
    return ['success' => true, 'questions' => $questions, 'count' => count($questions)];
}
...
if ($action === 'parsexml' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $result = parse_moodle_xml($xmlcontent);
