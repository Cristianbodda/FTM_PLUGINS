...
$areaNames = [
    ...
    // LOGISTICA - Aree A-H
    'LO_A' => ['name' => 'A. Organizzazione dei mandati logistici', 'icon' => '📋', 'color' => '#3498db', 'sector' => 'LOGISTICA'],
    'LO_B' => ['name' => 'B. Qualità ed efficienza dei processi', 'icon' => '⚡', 'color' => '#e74c3c', 'sector' => 'LOGISTICA'],
    'LO_C' => ['name' => 'C. Ricezione, controllo e stoccaggio merce', 'icon' => '📦', 'color' => '#2ecc71', 'sector' => 'LOGISTICA'],
    'LO_D' => ['name' => 'D. Commissionamento, preparazione e spedizione', 'icon' => '🚚', 'color' => '#f39c12', 'sector' => 'LOGISTICA'],
    'LO_E' => ['name' => 'E. Accettazione invii e consulenza', 'icon' => '🤝', 'color' => '#9b59b6', 'sector' => 'LOGISTICA'],
    'LO_F' => ['name' => 'F. Recapito e servizi logistici', 'icon' => '📬', 'color' => '#1abc9c', 'sector' => 'LOGISTICA'],
    'LO_G' => ['name' => 'G. Operazioni di magazzino', 'icon' => '🏭', 'color' => '#e67e22', 'sector' => 'LOGISTICA'],
    'LO_H' => ['name' => 'H. Commissionamento e carico', 'icon' => '📤', 'color' => '#34495e', 'sector' => 'LOGISTICA'],

    // ELETTRICITÀ - Aree per profilo
    'PE' => ['name' => 'Pianificazione & Progettazione', 'icon' => '📐', 'color' => '#3498db', 'sector' => 'ELETTRICITÀ'],
    'IE' => ['name' => 'Installazione impianti BT', 'icon' => '🔌', 'color' => '#e74c3c', 'sector' => 'ELETTRICITÀ'],
    'EM' => ['name' => 'Montaggio & Cablaggio quadri', 'icon' => '⚡', 'color' => '#f39c12', 'sector' => 'ELETTRICITÀ'],
    'ER' => ['name' => 'Reti di distribuzione MT/BT', 'icon' => '🔋', 'color' => '#9b59b6', 'sector' => 'ELETTRICITÀ'],

    // AUTOMAZIONE - 8 Aree (A-H) per entrambi i profili OA/MA
    'AU_A' => ['name' => 'A. Pianificazione & Documentazione', 'icon' => '📋', 'color' => '#3498db', 'sector' => 'AUTOMAZIONE'],
    'AU_B' => ['name' => 'B. Montaggio meccanico & Elettromeccanico', 'icon' => '🔩', 'color' => '#e67e22', 'sector' => 'AUTOMAZIONE'],
    'AU_C' => ['name' => 'C. Cablaggio elettrico & Quadri', 'icon' => '⚡', 'color' => '#f39c12', 'sector' => 'AUTOMAZIONE'],
    'AU_D' => ['name' => 'D. Automazione & PLC', 'icon' => '🤖', 'color' => '#2ecc71', 'sector' => 'AUTOMAZIONE'],
    'AU_E' => ['name' => 'E. Strumentazione & Misure', 'icon' => '📏', 'color' => '#9b59b6', 'sector' => 'AUTOMAZIONE'],
    'AU_F' => ['name' => 'F. Reti & Comunicazione industriale', 'icon' => '🌐', 'color' => '#1abc9c', 'sector' => 'AUTOMAZIONE'],
    'AU_G' => ['name' => 'G. Qualità & Sicurezza', 'icon' => '🛡️', 'color' => '#e74c3c', 'sector' => 'AUTOMAZIONE'],
    'AU_H' => ['name' => 'H. Manutenzione & Service', 'icon' => '🔧', 'color' => '#34495e', 'sector' => 'AUTOMAZIONE'],

    // METALCOSTRUZIONE - Aree per profilo
    'MC' => ['name' => 'Metalcostruttore', 'icon' => '🔩', 'color' => '#607d8b', 'sector' => 'METALCOSTRUZIONE'],
    'DF' => ['name' => 'Disegnatore Fabbricante', 'icon' => '📐', 'color' => '#795548', 'sector' => 'METALCOSTRUZIONE'],
 ...
 * Formato LOGISTICA: LOGISTICA_LO_A1 -> estrae LO_A
 */
function extract_area_from_code($code) {
    $parts = explode('_', $code);
    
    // Gestione speciale per LOGISTICA: LOGISTICA_LO_A1 -> LO_A
    if (count($parts) >= 3 && $parts[0] === 'LOGISTICA' && $parts[1] === 'LO') {
        $areaLetter = substr($parts[2], 0, 1);
        return 'LO_' . $areaLetter;
    }
    
    // Gestione speciale per AUTOMAZIONE: AUTOMAZIONE_OA_A1 -> AU_A
    if (count($parts) >= 3 && $parts[0] === 'AUTOMAZIONE') {
        $areaLetter = substr($parts[2], 0, 1);
        return 'AU_' . $areaLetter;
    }
    
    // Logica standard per altri settori (MECCANICA, AUTOMOBILE, CHIMFARM)
    if (count($parts) >= 2) {
        return $parts[1];
    }
    return 'OTHER';
