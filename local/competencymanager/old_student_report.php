...
echo '<a href="' . new moodle_url('/local/competencymanager/reports.php', ['courseid' => $courseid]) . '" class="btn btn-secondary">← Torna alla lista</a>';
echo '<div><button type="button" class="btn btn-primary mr-2" data-toggle="modal" data-target="#printModal">🖨️ Stampa</button>';
echo '<a href="' . new moodle_url('/local/competencymanager/export.php', ['userid' => $userid, 'courseid' => $courseid, 'format' => 'csv']) . '" class="btn btn-success mr-2">📥 CSV</a>';
echo '<a href="' . new moodle_url('/local/competencymanager/export.php', ['userid' => $userid, 'courseid' => $courseid, 'format' => 'excel']) . '" class="btn btn-success">📥 Excel</a></div></div></div>';
...
echo '<div class="modal-body"><form id="printForm" method="get" target="_blank" action="' . new moodle_url('/local/competencymanager/student_report.php') . '">';
...
foreach ($tabs as $tabKey => $tabLabel) {
    ...
    $url = new moodle_url('/local/competencymanager/student_report.php', array_merge(['userid' => $userid, 'courseid' => $courseid, 'tab' => $tabKey], $selectedArea ? ['selectedarea' => $selectedArea] : []));
