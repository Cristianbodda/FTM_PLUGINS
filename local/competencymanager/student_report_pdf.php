...
function pdf_render_valutazione($pdf, $p) {
    ...
    $html .= '<span style="font-size:14pt; color:' . $ev['color'] . '; font-weight:bold;">' . $ev['icon'] . ' VALUTAZIONE: ' . s($ev['label']) . '</span><br/>';
...
function pdf_render_radar_aree($pdf, $p) {
    ...
    foreach ($p['areasData'] as $area) {
        $radarData[] = ['label' => $area['icon'] . ' ' . $area['name'], 'value' => $area['percentage']];
    }
    ...
    foreach ($p['areasData'] as $area) {
        ...
        $html .= '<tr><td><strong>' . $area['icon'] . ' ' . s($area['name']) . '</strong></td>';
...
function pdf_render_radar_dettagli($pdf, $p) {
    ...
    foreach ($p['printRadarAreas'] as $areaCode) {
        ...
        pdf_section_title($pdf, $area['icon'] . ' DETTAGLIO: ' . $area['name'], $areaColor, true);
        ...
        $svg = generate_svg_radar($areaComps, $area['icon'] . ' ' . $area['name'] . ' - ' . $area['percentage'] . '%', 320, $areaColor . '40', $areaColor);
        ...
        foreach ($area['competencies'] as $comp) {
            ...
            $html .= '<td style="color:' . $band['color'] . '; font-weight:bold;">' . $band['icon'] . ' ' . s($band['label']) . '</td></tr>';
        }
        $html .= '</table><br/>';
        $pdf->writeHTML($html, true, false, true, false, '');
    }
...
function pdf_render_piano($pdf, $p) {
    ...
    foreach ($cats as $key => $cat) {
        ...
        $html = '<table cellpadding="4" style="width:100%; font-size:9pt; border:1px solid #dee2e6;">';
        $html .= '<tr style="background-color:#ecf0f1;"><th>Competenza</th><th style="text-align:center;">Azione</th><th style="text-align:center;">Risultato</th></tr>';
...
function pdf_render_dettagli($pdf, $p) {
    ...
    $html .= '<tr style="background-color:#34495e; color:#ffffff;">';
    $html .= '<th width="4%">#</th><th width="8%">Area</th><th width="12%">Codice</th>';
    $html .= '<th width="36%">Competenza</th><th width="12%" style="text-align:center;">Risposte</th>';
    $html .= '<th width="10%" style="text-align:center;">%</th><th width="18%">Valutazione</th></tr>';
    ...
    foreach ($p['competencies'] as $comp) {
        $code = $comp['idnumber'] ?: $comp['name'];
        $parts = explode('_', $code);
        ...
        $html .= '<td><span style="background-color:' . $areaInfo['color'] . '; color:#fff; padding:1px 4px; font-weight:bold; font-size:6pt;">' . ($areaInfo['icon'] ?? '') . ' ' . $aCode . '</span></td>';
...
function pdf_render_dual_legend($pdf, $legendData) {
    $html = '<table cellpadding="3" style="width:100%; font-size:7pt; border:1px solid #dee2e6;">';
    $html .= '<tr><th style="background-color:#34495e; color:#fff;">Area</th>';
    $html .= '<th style="background-color:#667eea; color:#fff; text-align:center; width:55px;">Auto</th>';
    $html .= '<th style="background-color:#28a745; color:#fff; text-align:center; width:55px;">Reale</th>';
    $html .= '<th style="background-color:#34495e; color:#fff; text-align:center; width:50px;">Gap</th></tr>';
    ...
    foreach ($legendData as $item) {
        ...
        $html .= '<td style="border:1px solid #dee2e6;"><strong>' . $item['icon'] . '</strong> ' . s($item['name']) . '</td>';
...
function pdf_render_dual_radar($pdf, $p) {
    ...
    foreach ($autoAreas as $area) {
        $autoRadar[] = ['label' => $area['icon'] . ' ' . $area['name'], 'value' => $area['percentage']];
    }
    ...
    foreach ($areasData as $area) {
        $perfRadar[] = ['label' => $area['icon'] . ' ' . $area['name'], 'value' => $area['percentage']];
    }
...
function pdf_render_gap_analysis($pdf, $p) {
    ...
    $html .= '<tr><th style="background-color:#34495e; color:#fff;">Competenza</th>';
    $html .= '<th style="background-color:#667eea; color:#fff; text-align:center; width:60px;">Auto</th>';
    $html .= '<th style="background-color:#28a745; color:#fff; text-align:center; width:60px;">Reale</th>';
    $html .= '<th style="background-color:#34495e; color:#fff; text-align:center; width:60px;">Gap</th>';
    $html .= '<th style="background-color:#34495e; color:#fff; text-align:center; width:100px;">Analisi</th></tr>';
...
function pdf_render_spunti($pdf, $p) {
    ...
    if (!empty($hints['positivi'])) {
        ...
        $html .= '<th style="background-color:#d5f5e3; color:#155724; text-align:center; width:70px;">Stato</th>';
        $html .= '<th style="background-color:#d5f5e3; color:#155724;">Note</th></tr>';
        foreach (array_slice($hints['positivi'], 0, 8) as $h) {
            $html .= '<tr style="background-color:#f8fff9;"><td style="border:1px solid #ddd;">' . s($h['competenza']) . '</td>';
            $html .= '<td style="border:1px solid #ddd; text-align:center; color:#28a745; font-weight:bold;">' . $h['gap']['icona'] . '</td>';
            $html .= '<td style="border:1px solid #ddd; font-size:8pt;">' . s($h['messaggio']) . '</td></tr>';
        }
        $html .= '</table><br/>';
        $pdf->writeHTML($html, true, false, true, false, '');
    }
...
function pdf_render_suggerimenti($pdf, $p) {
    ...
    if (!empty($allin)) {
        ...
        $html .= '<table cellpadding="4" style="width:100%; font-size:9pt; border:1px solid #dee2e6;">';
        $html .= '<tr><th style="background-color:#d4edda; color:#155724;">Area</th>';
        $html .= '<th style="background-color:#d4edda; color:#155724; text-align:center; width:60px;">Gap</th>';
        $html .= '<th style="background-color:#d4edda; color:#155724;">Attivita</th>';
        $html .= '<th style="background-color:#d4edda; color:#155724;">Ruoli</th></tr>';
...
function pdf_render_overlay_radar($pdf, $p) {
    ...
    // Comparison table
    $html = '<table cellpadding="4" style="width:100%; font-size:8pt; border:1px solid #dee2e6;">';
    $html .= '<tr style="background-color:#f0f4f8;"><th>Area</th>';
    $html .= '<th style="text-align:center; color:#28a745;">Rilevamento</th>';
    if (!empty($hasAuto)) $html .= '<th style="text-align:center; color:#667eea;">Autovalutazione</th>';
    if (!empty($hasCoach)) $html .= '<th style="text-align:center; color:#dc3545;">Formatore</th>';
    $html .= '<th style="text-align:center;">Media</th><th style="text-align:center;">Gap Max</th></tr>';

    for ($i = 0; $i < count($labels); $i++) {
        $vals = [$rilev[$i]];
        if (!empty($hasAuto)) $vals[] = ($oAuto[$i] ?? 0);
        if (!empty($hasCoach)) $vals[] = ($oCoach[$i] ?? 0);
        $valid = array_filter($vals, function($v) { return $v > 0; });
