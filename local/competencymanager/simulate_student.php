 ...
 * 
 * CHANGELOG:
 * 04/01/2026 - FIX CRITICO: Corretti campi tabella selfassessment
 *            - FIX: Aggiunta creazione assegnazioni in local_selfassessment_assign
 *            - FIX: Trigga observer dopo simulazione quiz
 */
...
<div class="sim-header">
    <h2>🤖 Simulatore Studente Universale <span class="fix-badge">v2.0 FIX</span></h2>
    <p>Simula automaticamente lo svolgimento di quiz e autovalutazioni per verificare il corretto funzionamento del sistema.</p>
    <p style="font-size: 12px; opacity: 0.8;">✅ Fix 04/01/2026: Corretto creazione assegnazioni e campi tabella</p>
</div>
...
/**
 * Crea assegnazioni per autovalutazione basate sulle competenze del quiz
 * NUOVO: Questa funzione simula quello che fa l'observer
 * 
 * @param int $quizid ID del quiz
 * @param int $userid ID utente
 * @return array Risultato operazione
 */
function create_selfassessment_assignments($quizid, $userid) {
    global $DB;
    ...
    if (empty($questions)) {
        $results['details'][] = "⚠️ Nessuna domanda trovata nel quiz";
        return $results;
    }
    ...
    $mappings = $DB->get_records_sql("
        ...
        FROM {qbank_competenciesbyquestion}
        WHERE questionid $sql_in
    ", $params);
    
    if (empty($mappings)) {
        $results['details'][] = "⚠️ Nessuna competenza associata alle domande";
        return $results;
    }
    ...
    foreach ($mappings as $mapping) {
        ...
        try {
            ...
            $results['details'][] = "⚠️ Errore assegnazione competenza $competencyid: " . $e->getMessage();
        }
    }
    
    $results['success'] = true;
    $results['details'][] = "✅ Assegnazioni create: {$results['assigned']}, già esistenti: {$results['skipped']}";
 ...
 * AGGIORNATO: Ora crea anche le assegnazioni per autovalutazione
 */
function simulate_quiz_attempt($quizid, $userid, $randomize = true) {
    ...
    try {
        ...
        // ========================================
        // NUOVO: Crea assegnazioni autovalutazione
        // ========================================
        $assign_result = create_selfassessment_assignments($quizid, $userid);
        $results['assignments_created'] = $assign_result['assigned'];
        $results['details'][] = "📋 " . implode(", ", $assign_result['details']);
 ...
 * CORRETTO: Usa i campi corretti della tabella (level, non bloomlevel; no courseid)
 * AGGIORNATO: Prima crea assegnazioni se non esistono
 */
function simulate_selfassessment($courseid, $userid, $randomize = true) {
    ...
    try {
        ...
        // Ottieni competenze testate nei quiz del corso (quelle con domande)
        ...
        if (empty($competencies)) {
            // Fallback: prova con competenze del corso
            $competencies = $DB->get_records_sql("
                SELECT DISTINCT c.id, c.shortname, c.idnumber
                FROM {competency} c
        ...
        if (empty($competencies)) {
            $results['message'] = "⚠️ Nessuna competenza trovata per il corso";
            return $results;
        }

        $results['details'][] = "🎯 Competenze trovate: " . count($competencies);
        ...
        foreach ($competencies as $comp) {
            ...
            // ========================================
            // PASSO 1: Crea assegnazione se non esiste
            // ========================================
            $assign_exists = $DB->record_exists('local_selfassessment_assign', [
                'userid' => $userid,
            ...
            if (!$assign_exists) {
                ...
                $assign->source = 'simulator';
                $assign->sourceid = $courseid;
                $assign->timecreated = $now;
            ...
            // ========================================
            // PASSO 2: Crea/aggiorna autovalutazione
            // CORRETTO: usa 'level' non 'bloomlevel', senza 'courseid'
            // ========================================
            ...
            if ($existing) {
                // Aggiorna esistente
                $existing->level = $level;  // CORRETTO: era bloomlevel
                ...
                $record->level = $level;  // CORRETTO: era bloomlevel
                // RIMOSSO: $record->courseid - campo non esiste nella tabella!
                $record->timecreated = $now;
        ...
        $results['assignments_created'] = $assigned;
        $results['details'][] = "📋 Assegnazioni create: $assigned";
        $results['details'][] = "✅ Competenze valutate: $rated";
...
/**
 * Verifica stato sistema autovalutazioni
 */
function check_selfassessment_status() {
    global $DB;
    
    $status = [];
    
    // Conta assegnazioni
    $status['assignments'] = $DB->count_records('local_selfassessment_assign');
    
    // Conta autovalutazioni
    $status['selfassessments'] = $DB->count_records('local_selfassessment');
...
if (!$courseid) {
    ...
    $sa_status = check_selfassessment_status();
    ?>
    
    <!-- Stato Sistema Autovalutazioni -->
    <div class="sim-card" style="background: linear-gradient(135deg, #00b894, #00cec9); color: white;">
        <h3 style="color: white; border-color: rgba(255,255,255,0.3);">📊 Stato Sistema Autovalutazioni</h3>
        <div style="display: flex; gap: 30px; flex-wrap: wrap;">
            <div>
                <strong style="font-size: 24px;"><?php echo $sa_status['assignments']; ?></strong>
                <br><small>Assegnazioni</small>
            </div>
            <div>
                <strong style="font-size: 24px;"><?php echo $sa_status['selfassessments']; ?></strong>
                <br><small>Autovalutazioni compilate</small>
            </div>
            <?php foreach ($sa_status['by_source'] as $src): ?>
            <div>
                <strong style="font-size: 18px;"><?php echo $src->count; ?></strong>
                <br><small>da <?php echo $src->source; ?></small>
            </div>
            <?php endforeach; ?>
        </div>
    </div>
    ...
    <div class="sim-card">
        ...
        if ($result['success']) {
            ...
            echo '<p><strong>Assegnazioni autovalutazione create:</strong> ' . $result['assignments_created'] . '</p>';
            echo '</div>';
        } else {
    ...
    <div class="sim-card">
        ...
        if ($result['success']) {
            ...
            echo '<p><strong>Assegnazioni create:</strong> ' . $result['assignments_created'] . '</p>';
            echo '</div>';
        } else {
            echo '<div class="error-box"><h4>' . $result['message'] . '</h4></div>';
        }
    ...
    <div class="sim-card">
        ...
        foreach ($quizzes as $quiz) {
            ...
            if ($result['success']) {
                ...
                echo ' <small>(+' . $result['assignments_created'] . ' assegnazioni)</small>';
                $completed++;
        ...
        if ($sa['success']) {
            echo '<span class="status-ok">✅ ' . $sa['competencies_rated'] . ' competenze</span>';
            echo ' <small>(+' . $sa['assignments_created'] . ' assegnazioni)</small>';
            $total_assignments += $sa['assignments_created'];
        } else {
            echo '<span class="status-warning">⚠️ ' . $sa['message'] . '</span>';
        }
        echo '</div>';
        ?>
        ...
        <div class="result-box">
            ...
            <p>Assegnazioni totali create: <strong><?php echo $total_assignments; ?></strong></p>
        </div>
    ...
    <div class="sim-card">
        ...
        foreach ($courses as $course_item) {
            ...
            // Autovalutazione
            $sa = simulate_selfassessment($cid, $student->id, true);
            $total_assignments += $sa['assignments_created'] ?? 0;
        ...
        <div class="result-box">
            ...
            <p>Assegnazioni create: <strong><?php echo $total_assignments; ?></strong></p>
        </div>
    ...
    <div class="sim-card">
        ...
        <p><small>Crea assegnazioni + compila autovalutazioni per tutte le competenze del corso</small></p>
    ...
    <div class="sim-card">
        ...
        <p><small>✅ Crea automaticamente assegnazioni per ogni quiz completato</small></p>
    ...
    <div class="sim-card">
        ...
        <a href="coverage_report.php" class="btn btn-danger">📊 Coverage Manager</a>
    ...
    function simQuiz(qid) {
        ...
        if (confirm('Simulare il quiz?\n\n✅ Verranno create anche le assegnazioni per autovalutazione')) location.href='?courseid=<?php echo $courseid; ?>&action=simulate_quiz&quizid='+qid+'&userid='+u+'&confirm=1';
    }
    function simSelfAssess() {
        var u = getStudent(); if (!u) return;
        if (confirm('Simulare autovalutazione?\n\n✅ Verranno create assegnazioni + autovalutazioni')) location.href='?courseid=<?php echo $courseid; ?>&action=simulate_selfassess&userid='+u+'&confirm=1';
    }
    function simAll() {
        var u = getStudent(); if (!u) return;
        if (confirm('⚠️ Simulare TUTTI i quiz + autovalutazione?\n\n✅ Verranno create tutte le assegnazioni automaticamente')) location.href='?courseid=<?php echo $courseid; ?>&action=simulate_all&userid='+u+'&confirm=1';
    }
    </script>
