...
<div class="check-card">
    ...
    foreach ($plugins as $plugin) {
        $results['total']++;
        $fullpath = $CFG->dirroot . $plugin['path'];
        ...
        if (file_exists($version_file)) {
            ...
            $version = isset($plugin_obj->release) ? $plugin_obj->release : 
                      (isset($plugin_obj->version) ? $plugin_obj->version : 'N/A');
            
            $results['passed']++;
            ?>
            <div class="plugin-status plugin-ok">
                <span class="plugin-icon">✅</span>
                <span class="plugin-name"><?php echo $plugin['name']; ?> <small style="color:#666;">(<?php echo $plugin['type']; ?>)</small></span>
