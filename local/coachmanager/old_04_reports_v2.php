...
foreach ($areas_data as $area_key => $area) {
    $sector = strtoupper($area['info']['settore'] ?? 'ALTRO');
    // Normalizza il settore
    if (strpos($area_key, 'automobile') !== false || strpos($sector, 'AUTOMOBILE') !== false) {
        $sector = 'AUTOMOBILE';
    } elseif (strpos($area_key, 'meccanica') !== false || strpos($sector, 'MECCANICA') !== false) {
        $sector = 'MECCANICA';
    } elseif (strpos($area_key, 'logistica') !== false || strpos($sector, 'LOGISTICA') !== false) {
        $sector = 'LOGISTICA';
    } elseif (strpos($area_key, 'chimfarm') !== false || strpos($sector, 'CHIMFARM') !== false) {
        $sector = 'CHIMFARM';
    } elseif (strpos($area_key, 'elettricita') !== false || strpos($sector, 'ELETTRICITA') !== false) {
        $sector = 'ELETTRICITA';
    } elseif (strpos($area_key, 'automazione') !== false || strpos($sector, 'AUTOMAZIONE') !== false) {
        $sector = 'AUTOMAZIONE';
    } elseif (strpos($area_key, 'metalcostruzione') !== false || strpos($sector, 'METALCOSTRUZIONE') !== false) {
        $sector = 'METALCOSTRUZIONE';
    }
    ...
    $all_sectors[$sector] = $sectors_data[$sector]['display_name'];
}
...
$radar_labels = json_encode(array_map(fn($a) => $a['info']['nome'], $filtered_areas_data));
$radar_quiz_data = json_encode(array_map(fn($a) => $a['quiz_media'] ?? 0, $filtered_areas_data));
$radar_autoval_data = json_encode(array_map(fn($a) => $a['autoval_media'] ?? 0, $filtered_areas_data));
                ...
                <div class="radar-controls">
                    ...
                    <div class="control-group">
                        ...
                        <select id="filterSector" onchange="updateRadar()">
                            ...
                            <?php foreach ($all_sectors as $code => $name): ?>
                            <option value="<?php echo strtolower($code); ?>"><?php echo $name; ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                </div>
...
function initRadarCharts() {
    ...
    const labels = Object.values(areasData).slice(0, 8).map(a => a.info.nome.substring(0, 15));
    const quizData = Object.values(areasData).slice(0, 8).map(a => a.quiz_media || 0);
    const autovalData = Object.values(areasData).slice(0, 8).map(a => a.autoval_media || 0);
