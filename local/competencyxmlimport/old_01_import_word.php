 ...
 * Il formato .docx è un archivio ZIP contenente XML
 */
function extract_text_from_docx($filepath) {
    ...
    if ($zip->open($filepath) === true) {
        // Il contenuto principale è in word/document.xml
        $xml = $zip->getFromName('word/document.xml');
        $zip->close();
        
        if ($xml) {
            // Rimuovi namespace per semplificare il parsing
            $xml = str_replace(['w:', 'wp:', 'a:'], '', $xml);
            
            // Parse XML
            $dom = new DOMDocument();
            @$dom->loadXML($xml);
            
            // Estrai testo da tutti i nodi <t> (text)
            $xpath = new DOMXPath($dom);
            $paragraphs = $dom->getElementsByTagName('p');
            
            foreach ($paragraphs as $p) {
                $text_nodes = $p->getElementsByTagName('t');
                $para_text = '';
                foreach ($text_nodes as $t) {
                    $para_text .= $t->nodeValue;
                }
                if (trim($para_text)) {
                    $content .= trim($para_text) . "\n";
                }
            }
        }
    }
...
function parse_questions_from_text($text) {
    ...
    for ($i = 0; $i < count($lines); $i++) {
        ...
        // Nuovo numero domanda: **Q01**, Q01, **Q1**, etc.
        ...
        // Risposte A), B), C), D) o A., B., C., D.
        if (preg_match('/^([A-D])[\.\)]\s*(.+)$/i', $line, $m)) {
            $letter = strtoupper($m[1]);
            $current['answers'][$letter] = trim($m[2]);
            continue;
        }
        
        // Risposte inline: A) ... B) ... C) ... D) ...
        if (preg_match('/A[\.\)]\s*.+\s+B[\.\)]\s*.+/i', $line)) {
        ...
        // Competenza (vari formati)
        // Formato 1: Competenza: SETTORE_AREA_NUM
        // Formato 2: Codice competenza: SETTORE_AREA_NUM
        // Formato 3: Codice: SETTORE_AREA_NUM
        // Formato 4: **Competenza:** SETTORE_AREA_NUM
        if (preg_match('/(?:Competenza|Codice(?:\s+competenza)?)\s*:\s*([A-Z][A-ZÀÈÉÌÒÙ0-9_]+)/i', $line, $m)) {
            $current['competency'] = strtoupper(trim($m[1]));
            continue;
        }
        
        // Risposta corretta (vari formati)
        // Formato 1: Risposta corretta: A
        // Formato 2: ✓ Risposta corretta: A
        // Formato 3: Risposta: A
        ...
        if (!empty($line) && empty($current['question'])) {
            // Rimuovi grassetto markdown
            $line = preg_replace('/\*\*(.+?)\*\*/', '$1', $line);
            $current['question'] = $line;
        } else if (!empty($line) && !empty($current['question']) && 
                   !preg_match('/^[A-D][\.\)]/', $line) &&
                   strpos($line, 'ompetenza') === false &&
                   strpos($line, 'isposta') === false) {
