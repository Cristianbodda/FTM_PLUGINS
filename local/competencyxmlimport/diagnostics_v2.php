 ...
 * AUTOMOBILE: area = terza parte (A, B, C...) es. AUTOMOBILE_MR_A1 → area A
 * MECCANICA: area = seconda parte (LMB, CNC...) es. MECCANICA_LMB_01 → area LMB
 */
function get_sector_areas($frameworkid, $sector) {
    ...
    foreach ($competencies as $c) {
        ...
        if ($sector == 'AUTOMOBILE') {
            // AUTOMOBILE_MR_A1 → area = A (prendi prima lettera della terza parte)
            // AUTOMOBILE_MAu_H2 → area = H
            if (count($parts) >= 3) {
                // Estrai la lettera dell'area (prima lettera della terza parte)
                $thirdPart = $parts[2];
                preg_match('/^([A-Z])/i', $thirdPart, $matches);
                $areaCode = isset($matches[1]) ? strtoupper($matches[1]) : $thirdPart;
            } else {
                $areaCode = 'OTHER';
            }
        } else {
        ...
        if (!isset($areas[$areaCode])) {
            $areas[$areaCode] = [
                'code' => $areaCode,
                'name' => get_area_name($areaCode, $sector),
                'competencies' => [],
                'count' => 0
            ];
        }
 ...
 */
function get_area_name($code, $sector = '') {
    // Aree AUTOMOBILE (A-N)
    $automobile_areas = [
    ...
    $meccanica_areas = [
        ...
        'CNC' => 'CNC e tecnologie digitali',
        'ASS' => 'Assemblaggio e montaggio',
        'MIS' => 'Misurazione e controllo qualità',
        'GEN' => 'Trattamenti e processi speciali',
        'MAN' => 'Manutenzione e revisione',
    ...
    // Cerca prima nel settore specifico
    if ($sector == 'AUTOMOBILE' && isset($automobile_areas[$code])) {
        return $automobile_areas[$code];
    }
    if ($sector == 'MECCANICA' && isset($meccanica_areas[$code])) {
        return $meccanica_areas[$code];
    }
    
    // Fallback su tutte le mappe
    if (isset($meccanica_areas[$code])) return $meccanica_areas[$code];
    if (isset($automobile_areas[$code])) return $automobile_areas[$code];
    if (isset($other_areas[$code])) return $other_areas[$code];
 ...
 * Icona per area
 */
function get_area_icon($code, $sector = '') {
    // Icone AUTOMOBILE (A-N)
    $automobile_icons = [
        'A' => '📋', 'B' => '🔧', 'C' => '💧', 'D' => '💨',
        'E' => '⚙️', 'F' => '🛞', 'G' => '🔌', 'H' => '📡',
        'I' => '❄️', 'J' => '⚡', 'K' => '🚗', 'L' => '🛡️',
    ...
    if ($sector == 'AUTOMOBILE' && isset($automobile_icons[$code])) {
        return $automobile_icons[$code];
    }
    if (isset($meccanica_icons[$code])) return $meccanica_icons[$code];
    if (isset($automobile_icons[$code])) return $automobile_icons[$code];
 ...
 * AUTOMOBILE: cerca AUTOMOBILE_MR_A% o AUTOMOBILE_MAu_A% per area A
 * MECCANICA: cerca MECCANICA_LMB_% per area LMB
 */
function count_area_questions($frameworkid, $sector, $areacode, $contextid) {
    global $DB;
    
    if ($sector == 'AUTOMOBILE') {
        // Per AUTOMOBILE, l'area è la lettera (A, B, C...)
        // Dobbiamo cercare AUTOMOBILE_MR_A% e AUTOMOBILE_MAu_A%
        $sql = "SELECT COUNT(DISTINCT q.id) as cnt
                ...
                AND (c.idnumber LIKE ? OR c.idnumber LIKE ?)
                AND qc.contextid = ?";
        
        $pattern1 = $sector . '_MR_' . $areacode . '%';
        $pattern2 = $sector . '_MAu_' . $areacode . '%';
        $result = $DB->get_record_sql($sql, [$frameworkid, $pattern1, $pattern2, $contextid]);
    } else {
        // Per altri settori (MECCANICA, ecc.)
        $sql = "SELECT COUNT(DISTINCT q.id) as cnt
                FROM {question} q
                JOIN {qbank_competenciesbyquestion} qbc ON qbc.questionid = q.id
                JOIN {competency} c ON c.id = qbc.competencyid
        ...
        <div class="areas-grid">
            ...
            <div class="area-card">
                <div class="area-card-header">
                    <div class="area-icon"><?php echo get_area_icon($areaCode, $sector); ?></div>
                    <div class="area-info">
                        <span class="area-code"><?php 
                            if ($sector == 'AUTOMOBILE') {
                                echo 'Area ' . $areaCode;
                            } else {
                                echo $sector . '_' . $areaCode;
                            }
                        ?></span>
                ...
                <div class="area-card-footer">
                    ...
                    <a href="<?php echo $CFG->wwwroot; ?>/question/edit.php?courseid=<?php echo $courseid; ?>" target="_blank" title="Question Bank">
                        ❓ Domande
                    </a>
                    <a href="scheda_competenze.php?courseid=<?php echo $courseid; ?>&action=assign" title="Assegna Competenze">
                        🔗 Assegna
                    </a>
                </div>
            </div>
        ...
        <div class="scrollable">
            ...
            <div style="margin-bottom: 25px;">
                <h4 style="display: flex; align-items: center; gap: 10px; margin-bottom: 15px; padding: 10px; background: #f8f9fa; border-radius: 8px;">
                    <span style="font-size: 24px;"><?php echo get_area_icon($areaCode, $sector); ?></span>
                ...
                <div class="competency-list">
                    <?php foreach ($areaData['competencies'] as $comp): 
                        ...
                        $compDescription = strip_tags($comp->description ?? '');
                        $compDescription = html_entity_decode($compDescription, ENT_QUOTES, 'UTF-8');
                        $compDescription = trim($compDescription);
                        // Se descrizione vuota, usa shortname
                        if (empty($compDescription)) {
                            $compDescription = $comp->shortname;
                        }
                    ?>
                    <div class="competency-item" id="comp-<?php echo $comp->id; ?>">
                        <span class="competency-code"><?php echo $comp->idnumber; ?></span>
                        <span class="competency-name"><?php echo format_string($compDescription); ?></span>
                        ...
                        <div class="competency-actions">
                            ...
                            <?php else: ?>
                            <a href="scheda_competenze.php?courseid=<?php echo $courseid; ?>&action=assign&comp=<?php echo urlencode($comp->idnumber); ?>" 
                               class="action-btn assign" title="Assegna domande a questa competenza">
                                ➕ Assegna
                            </a>
                            <?php endif; ?>
                            <a href="<?php echo $CFG->wwwroot; ?>/admin/tool/lp/editcompetency.php?competencyframeworkid=<?php echo $frameworkid; ?>&id=<?php echo $comp->id; ?>" 
                               target="_blank" class="action-btn edit" title="Modifica competenza">
                                ✏️
                            </a>
                        </div>
                    </div>
                    ...
                    <div class="questions-sublist" id="questions-<?php echo $comp->id; ?>">
                        ...
                        <div class="question-item">
                            ...
                            <div class="question-actions">
                                <a href="<?php echo $CFG->wwwroot; ?>/question/bank/editquestion/question.php?courseid=<?php echo $courseid; ?>&id=<?php echo $q->id; ?>" 
                                   target="_blank" class="action-btn edit" title="Modifica domanda">
                                    ✏️
                                </a>
                                <a href="<?php echo $CFG->wwwroot; ?>/question/bank/previewquestion/preview.php?id=<?php echo $q->id; ?>&courseid=<?php echo $courseid; ?>" 
                                   target="_blank" class="action-btn preview" title="Anteprima">
                                    👁️
                                </a>
                            </div>
                        </div>
            ...
            <table class="data-table">
                <thead>
                    <tr>
                        ...
                        <th style="width: 25%;">Anteprima</th>
