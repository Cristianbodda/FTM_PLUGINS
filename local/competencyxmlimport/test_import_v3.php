...
echo "<h1>🧪 Test Import v3</h1>";
echo "<pre style='background:#f0f0f0; padding:20px;'>";

// XML
$xml = '<quiz>
<question type="multichoice">
    <n><text>TEST_V3_Q01</text></n>
...
echo "\n=== AGGIUNGI DOMANDE (API Moodle) ===\n\n";

// Usa quiz_add_quiz_question per aggiungere le domande
$quizobj = quiz::create($cm->id);

foreach ($question_ids as $qid) {
    quiz_add_quiz_question($qid, $quizobj->get_quiz(), 0, 1.0);
    echo "✅ Aggiunta domanda $qid\n";
}
...
foreach ($slots as $s) {
    ...
    if ($ref) {
        $qv = $DB->get_record('question_versions', ['questionbankentryid' => $ref->questionbankentryid], '*', IGNORE_MULTIPLE);
        if ($qv) {
            $cnt = $DB->count_records('question_answers', ['question' => $qv->questionid]);
            echo "  Slot {$s->slot}: question {$qv->questionid} - $cnt risposte\n";
        }
    }
}
