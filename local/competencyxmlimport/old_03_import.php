...
<?php
        ...
        $category_id = $existing_cat ? $existing_cat->id : $DB->insert_record('question_categories', $category);
        
        // Import usando API Moodle
        ...
        if ($qformat->importpreprocess()) {
            $imported_questions = $qformat->readquestions($xml);
            if ($imported_questions) {
                foreach ($imported_questions as $q) {
                    if ($q->qtype == 'category') continue;
                    try {
                        $q->category = $category_id;
                        $q->createdby = $USER->id;
                        $q->modifiedby = $USER->id;
                        question_bank::get_qtype($q->qtype)->save_question($q, $q);
                        $imported++;
                    } catch (Exception $e) {
                        $errors[] = $e->getMessage();
                    }
        ...
        foreach ($imported_q as $q) {
            if (preg_match('/([A-Z]+_[A-Z0-9]+_[A-Z0-9]+)/', $q->name, $m)) {
                $comp_code = $m[1];
