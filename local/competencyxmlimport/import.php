...
elseif ($step == 4):
    ...
    $questions = $_SESSION['import_questions'];
    $quiz_name = $_SESSION['import_quiz_name'];
    $frameworkid = isset($_SESSION['import_frameworkid']) ? intval($_SESSION['import_frameworkid']) : 0;
    $sector = $_SESSION['import_sector'];
    
    // Se frameworkid non valido, prova a recuperarlo dal primo framework disponibile
    if ($frameworkid <= 0) {
        $first_fw = $DB->get_record_sql("SELECT id FROM {competency_framework} ORDER BY id LIMIT 1");
        if ($first_fw) {
            $frameworkid = $first_fw->id;
        }
    }
...
<?php
        ...
        foreach ($valid_questions as $q) {
            try {
                // Nome domanda con competenza (max 255 char)
                // Usa prefisso corto per evitare troncamento
                $prefix = substr(preg_replace('/[^A-Z0-9]/i', '', $quiz_name), 0, 15);
                $q_name = sprintf('%s_Q%02d_%s', $prefix, $q['number'], $q['competency']);
                ...
                // Assegna competenza
                // Prima cerca nel framework specificato
                $comp = $DB->get_record('competency', [
                    'idnumber' => $q['competency'],
                    'competencyframeworkid' => $frameworkid
                ]);
                
                // Se non trovata, cerca in qualsiasi framework (fallback)
                if (!$comp) {
                    $comp = $DB->get_record('competency', ['idnumber' => $q['competency']]);
                }
                ...
                if ($comp) {
                    ...
                    if (!$existing) {
                        ...
                        $assigned++; // Conta anche quelle già esistenti
                    }
                } else {
                    $errors[] = "Competenza {$q['competency']} non trovata in nessun framework";
                }
